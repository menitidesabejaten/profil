...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // B. Deteksi API Mobile (Bawaan browser modern untuk deteksi HP yang akurat)
    const isMobileAPI = navigator.userAgentData ? navigator.userAgentData.mobile : false;
    
    // C. Deteksi Dukungan Sentuhan (Hardware)
    const isTouch = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0);
    
    // D. Deteksi Lebar Layar Fisik Perangkat (Diubah max 767px agar Laptop resolusi min 1366x768 TIDAK terblokir)
    const isSmallPhysicalScreen = Math.min(window.screen.width, window.screen.height) <= 767;
    
    // LOGIKA BLOKIR KETAT: Jika terdeteksi murni sebagai HP, 
    // ATAU perangkat sentuh dengan layar seukuran HP (Situs Desktop HP)
    if (isMobileAPI || (isMobileUA && isSmallPhysicalScreen) || (isTouch && isSmallPhysicalScreen)) {
        // Blokir website
